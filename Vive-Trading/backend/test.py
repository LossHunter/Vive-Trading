...
"""
        ...
        # 검증 전 로그
        logger.info("=" * 80)
        logger.info("🔍 [TEST.PY] 첫 번째 검증 시작 - validate_trade_decision 호출 전")
        logger.info(f"  📋 전달할 파라미터 값:")
        logger.info(f"    - prompt_id: {prompt_data.id}")
        logger.info(f"    - account_id: {account_id}")
        logger.info(f"    - decision.coin: {validated_decision.coin}")
        logger.info(f"    - decision.signal: {validated_decision.signal}")
        logger.info(f"    - signal_created_at: {simulation_time}")
        logger.info(f"    - confidence: {_to_decimal(validated_decision.confidence)} (원본: {validated_decision.confidence}, type: {type(validated_decision.confidence)})")
        logger.info(f"    - justification: {validated_decision.justification} (length: {len(validated_decision.justification) if validated_decision.justification else 0})")
        logger.info(f"    - thinking: {thinking_part[:100] if thinking_part else None}... (length: {len(thinking_part) if thinking_part else 0}, type: {type(thinking_part)})")
        logger.info(f"    - full_prompt: {full_prompt_for_training[:100] if full_prompt_for_training else None}... (length: {len(full_prompt_for_training) if full_prompt_for_training else 0}, type: {type(full_prompt_for_training)})")
        logger.info(f"    - full_response: {full_response[:100] if full_response else None}... (length: {len(full_response) if full_response else 0}, type: {type(full_response)})")
        logger.info("=" * 80)
        ...
        logger.info("=" * 80)
        logger.info(f"🔍 [TEST.PY] 첫 번째 검증 완료 - 결과: is_valid={is_valid}, errors={validation_errors}")
        logger.info("=" * 80)
...
"""
                ...
                # 재요청 검증 전 로그
                logger.info("=" * 80)
                logger.info("🔍 [TEST.PY] 재요청 검증 시작 - validate_trade_decision 호출 전")
                logger.info(f"  📋 전달할 파라미터 값:")
                logger.info(f"    - prompt_id: {prompt_data.id}")
                logger.info(f"    - account_id: {account_id}")
                logger.info(f"    - retry_decision.coin: {retry_decision.coin}")
                logger.info(f"    - retry_decision.signal: {retry_decision.signal}")
                logger.info(f"    - signal_created_at: {simulation_time}")
                logger.info(f"    - confidence: {_to_decimal(retry_decision.confidence)} (원본: {retry_decision.confidence}, type: {type(retry_decision.confidence)})")
                logger.info(f"    - justification: {retry_decision.justification} (length: {len(retry_decision.justification) if retry_decision.justification else 0})")
                logger.info(f"    - thinking: {retry_thinking[:100] if retry_thinking else None}... (length: {len(retry_thinking) if retry_thinking else 0}, type: {type(retry_thinking)})")
                logger.info(f"    - full_prompt: {retry_full_prompt_for_training[:100] if retry_full_prompt_for_training else None}... (length: {len(retry_full_prompt_for_training) if retry_full_prompt_for_training else 0}, type: {type(retry_full_prompt_for_training)})")
                logger.info(f"    - full_response: {retry_raw_content[:100] if retry_raw_content else None}... (length: {len(retry_raw_content) if retry_raw_content else 0}, type: {type(retry_raw_content)})")
                logger.info("=" * 80)
                ...
                if retry_is_valid:
                    ...
                    # 재요청 검증 실패 시 전달된 파라미터 값 로그
                    logger.info("=" * 80)
                    logger.info("🔍 [TEST.PY] 재요청 검증 실패 - llm_trading_execution 저장 시 전달된 값:")
                    logger.info(f"  📋 저장 시 전달된 파라미터 값:")
                    logger.info(f"    - prompt_id: {prompt_data.id}")
                    logger.info(f"    - account_id: {account_id}")
                    logger.info(f"    - coin: {retry_decision.coin.upper() if retry_decision.coin else ''}")
                    logger.info(f"    - signal_type: {retry_decision.signal.lower().strip() if retry_decision.signal else ''}")
                    logger.info(f"    - execution_status: failed")
                    logger.info(f"    - failure_reason: {', '.join(retry_validation_errors)}")
                    logger.info(f"    - confidence: {_to_decimal(retry_decision.confidence)} (type: {type(_to_decimal(retry_decision.confidence))}, is None: {_to_decimal(retry_decision.confidence) is None})")
                    logger.info(f"    - justification: {retry_decision.justification[:100] if retry_decision.justification else None}... (length: {len(retry_decision.justification) if retry_decision.justification else 0}, is None: {retry_decision.justification is None})")
                    logger.info(f"    - thinking: {retry_thinking[:100] if retry_thinking else None}... (length: {len(retry_thinking) if retry_thinking else 0}, is None: {retry_thinking is None})")
                    logger.info(f"    - full_prompt: {retry_full_prompt_for_training[:100] if retry_full_prompt_for_training else None}... (length: {len(retry_full_prompt_for_training) if retry_full_prompt_for_training else 0}, is None: {retry_full_prompt_for_training is None})")
                    logger.info(f"    - full_response: {retry_raw_content[:100] if retry_raw_content else None}... (length: {len(retry_raw_content) if retry_raw_content else 0}, is None: {retry_raw_content is None})")
                    logger.info(f"    - signal_created_at: {simulation_time}")
                    logger.info("  ⚠️ 위 값들이 validate_trade_decision 내부의 _save_validation_failure로 전달됩니다.")
                    logger.info("=" * 80)
